    ...
    <div class="page-body">
        ...
        <div class="card">
            ...
            <div class="card-block">
                ...
                <button  class="btn btn-danger delete_all" data-url="{{ route('multiplechatsdelete') }}">Delete All Selected</button>
                @endcan
                ...
                <table id="demo-foo-filtering" class="table table-striped">
                    ...
                    <tr>
                        ...
                        <th data-breakpoints="xs"> <input type="checkbox" id="master"></th>
                    ...
                    @forelse($chats as $chat)
                        <tr id="tr_{{$chat->id}}">
                            <td><input type="checkbox" class="sub_chk" data-id="{{$chat->id}}"> </td>
                            <td>{{$loop->iteration}}</td>
                            <td>{{$chat->customer->name}}</td>
                            @if($chat->status() =='answered')
                            <td> 
                            @can('chat-edit')
                            <a type="button"       data-effect="effect-scale" data-toggle="modal" href="#edit_message" data-id_chat="{{$chat->id}}" data-sent_message="{{$chat->sent_message}}"  data-customer_name="{{$chat->customer->name}}"  class="dropdown-item waves-light waves-effect" >{{ \Illuminate\Support\Str::limit($chat->sent_message,20)}}</a>
                            @endcan
                            </td>
                            @else

                                <td> {{ \Illuminate\Support\Str::limit($chat->sent_message,20)}}</td>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <form method="POST" id="add-form" action= "{{route('Chat.destroy',1)}}">
                                @csrf
                                {{method_field('delete')}}
                                <div class="form-group">
